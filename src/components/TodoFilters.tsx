    ...
    return (
        <div className="bg-slate-800/30 backdrop-blur-md border-2 border-slate-600 pixel-border-glow p-4 md:p-6 hover:border-cyan-400 transition-all duration-300 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                {hasActiveFilters && (
                    <Button variant="danger" size="sm" onClick={onClearFilters} className="self-start md:self-auto">
            ...
            <div className="space-y-3">
                <label className="block text-sm font-bold text-cyan-300 font-mono tracking-wider">🎯 TRẠNG THÁI</label>
            ...
            <div className="pt-4 border-t-2 border-slate-600">
                <div className="bg-gradient-to-r from-purple-900/30 to-cyan-900/30 border border-purple-400 p-3 pixel-border-glow">
                    <p className="text-sm text-purple-300 font-mono text-center">
                        HIỂN THỊ <span className="font-bold text-cyan-400">{filteredCount}</span> /{" "}
                        <span className="font-bold text-yellow-400">{totalTodos}</span> NHIỆM VỤ
                        {hasActiveFilters && <span className="text-pink-400 font-bold"> (ĐÃ LỌC)</span>}
                    </p>
                </div>
            </div>
        </div>
