...
import { formatDate, formatDateTime, isOverdue, getPriorityColor, getPriorityLabel } from "@/utils"
...
export function TodoItem({ todo, onToggle, onDelete, onUpdate }: TodoItemProps) {
    ...
    return (
        <div
            className={`bg-gradient-to-r from-slate-800/50 to-slate-700/50 backdrop-blur-md border-2 pixel-border-glow p-4 md:p-6 transition-all duration-300 hover:shadow-[0_0_25px_rgba(6,182,212,0.3)] ${
            ...
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-2 md:gap-4">
                        <h3
                            className={`text-lg md:text-xl font-bold font-mono tracking-wide ${
                        ...
                        {/* Priority badge */}
                        <span
                            className={`inline-flex items-center px-3 py-1 border-2 text-xs font-bold font-mono tracking-wider self-start pixel-border-glow ${priorityColorClass}`}
                        >
                    ...
                    {todo.description && (
                        <p
                            className={`mt-2 text-sm font-mono ${todo.completed ? "text-gray-500" : "text-slate-300"} ${
